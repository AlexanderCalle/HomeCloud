...
function Collection() {
  ...
  const [fileIdShared, setFileIdShared] = React.useState(null)
  ...
  function renameFolder(e) {
    e.preventDefault();
    Axios({
      method: 'POST', url: `http://${process.env.REACT_APP_HOST_IP}:3030/folders/renamefolder/${folderId}`, data: {
        name: newFoldername,
      }
    }).then(async result => {
  ...
  function checkSharedFiles(fileId) {
    Axios.get(`http://${process.env.REACT_APP_HOST_IP}:3030/files/shared/${fileId}`)
      .then(res => {
        if (res.status === 200) {
          if (res.data.found == undefined) {
            deletefile(fileId);
          } else {
            setFileIdShared(fileId);
            setShowConfirmModal(true);
          }
        } else {
          console.log(res.status + ": error =" + res.data);
        }
      }).catch(err => console.log(err))
  }

  function deleteSharedFile(fileId) {
    Axios.get(`http://${process.env.REACT_APP_HOST_IP}:3030/files/shared/delete/${fileId}`)
      .then(res => {
        if (res.status === 200) {
          deletefile(fileId);
        } else {
            ...
            <div className="block items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
              ...
              <div className="inline-block rounded-xl text-left overflow-hidden bg-white shadow-xl transform align-middle sm:max-w-md sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                <div className="pb-2 sm:pb-4">
                  <div className="flex flex-col w-full">
                    <div className="flex flex-row justify-center items-center h-11 bg-cornblue-400 rounded-t-lg shadow-xl">
                      <h3 className="text-lg leading-6 font-medium text-cornblue-200" id="modal-headline">Confirm</h3>
                      <button onClick={() => setShowConfirmModal(false)} className="absolute right-2 text-cornblue-200 focus:outline-none">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                      </button>
                    </div>
                    <div className="mt-4 px-4">
                      Are you sure you want to delete a file that is also shared <br /><br />
                      <strong>Press delete to confirm</strong>
                    </div>
                  </div>
                </div>
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <button type="submit" onClick={() => deleteSharedFile(fileIdShared)} className="sm:w-28 w-full inline-flex justify-center rounded-lg shadow-sm px-6 py-2 bg-red-400 font-medium text-white hover:bg-red-600 focus:outline-none sm:ml-3 text-sm">
                    Delete
                  </button>
                  <button type="button" onClick={() => setShowConfirmModal(false)} className="mt-3 sm:mt-0 sm:w-28 w-full inline-flex justify-center rounded-md shadow-sm px-4 py-2 bg-cornblue-200 font-medium text-cornblue-600 focus:outline-none sm:ml-3 text-sm">
                    Cancel
                  </button>
                </div>
              </div>
